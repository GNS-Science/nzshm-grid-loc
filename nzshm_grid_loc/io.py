...
from nzshm_grid_loc.grids import Grid, get_precision
...
def load_zip(file_name: str) -> (str, io.BytesIO):
    """
...
def write_zip(file_name: str, data: Union[bytes, str]) -> None:
    """
...
def gdf_to_wkt_zip(gdf: GeoDataFrame, file_out: str, tolerance: float):
    ...
    data = sio.getvalue()
    data = data.replace(', ', ',')
    write_zip(file_out, data)


def data_to_base64_zip(data: str) -> str:
    """
    Returns a base64-encoded zipped version of the input string
    :param data: a string
    :return: a UTF-8 string that is the encoded version of the input
    """
    buffer = io.BytesIO()
    with zipfile.ZipFile(buffer, 'w', compression=zipfile.ZIP_LZMA) as zf:
        zf.writestr('data', data)
        zf.close()
    return base64.b64encode(buffer.getvalue()).decode("utf-8", "ignore")
...
def grid_to_base64(grid: Grid, lat_first=False) -> None:
    ...
    encoded = data_to_base64_zip(plain_grid.getvalue())
    print_b64_as_py(encoded)


def data_from_base64_zip(b64_data: str) -> str:
    """
    Decodes and unzips a base64-encoded zipped string.
    ...
    return plain_bytes.read().decode("utf-8", "ignore")
...
def latlon_from_base64_zip(b64_data: str) -> list:
    ...
    """
    data = data_from_base64_zip(b64_data)
    result = []
