...
from nzshm_grid_loc.io import write_grid, load_grid
from nzshm_grid_loc.geography import Regions
from nzshm_grid_loc.plot_grid import Plot


def generate_grid(file_name: str,
                  step: float,
                  lat_min=-48, lat_max=-34, lon_min=166, lon_max=179,
                  neighbours=2,
                  predicate=Regions.NZ_SMALL.predicate()) -> Grid:
    """
    ...
    grid = make_grid(lat_min, lat_max, lon_min, lon_max, step)
    grid = grid.filter(predicate)
...
def diff_grids(file_name_a: str, file_name_b: str) -> None:
    """
    Load two grids and create a visual diff
    :param file_name_a: file a
    :param file_name_b: file b
    :return: None
    """
    grid = load_grid(file_name_a)
    grid_new = load_grid(file_name_b)
